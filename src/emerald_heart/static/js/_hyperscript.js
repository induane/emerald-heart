...
(function (self, factory) {
    ...
    static tokenize(string, template) {
      ...
      while (position < source.length) {
        if (
          (currentChar() === "-" &&
            nextChar() === "-" &&
            (Lexer.isWhitespace(nextCharAt(2)) ||
              nextCharAt(2) === "" ||
              nextCharAt(2) === "-")) ||
          (currentChar() === "/" &&
            nextChar() === "/" &&
          ...
          if (Lexer.isWhitespace(currentChar())) {
            ...
            (!inTemplate() && Lexer.isIdentifierChar(currentChar()))
          ) {
        ...
        while (currentChar() && currentChar() !== startChar) {
          if (currentChar() === "\\") {
            ...
            if (nextChar === "b") {
              ...
              if (Number.isNaN(hex)) {
                throw Error(
                  "Invalid hexadecimal escape at " +
                    Lexer.positionString(string),
                );
              }
  ...
  class Tokens {
    ...
    peekToken(value, peek, type) {
      ...
      if (
        this.tokens[peek] &&
        this.tokens[peek].value === value &&
        this.tokens[peek].type === type
      ) {
    ...
    constructor(runtime) {
      ...
      this.addGrammarElement("feature", function (parser, runtime, tokens) {
        ...
        var featureDefinition =
          parser.FEATURES[tokens.currentToken().value || ""];
      ...
      this.addGrammarElement("command", function (parser, runtime, tokens) {
        ...
        var commandDefinition =
          parser.COMMANDS[tokens.currentToken().value || ""];
        let commandElement;
      ...
      this.addGrammarElement(
        "indirectExpression",
        function (parser, runtime, tokens, root) {
  ...
  class Runtime {
    ...
    constructor(lexer, parser) {
      ...
      this.parser =
        parser ??
        new Parser(this).use(hyperscriptCoreGrammar).use(hyperscriptWebGrammar);
    ...
    shouldAutoIterate(value) {
      return (
        (value != null && value[shouldAutoIterateSymbol]) ||
    ...
    processNode(elt) {
      ...
      if (elt.querySelectorAll) {
        this.forEach(
          elt.querySelectorAll(selector + ", [type='text/hyperscript']"),
          (elt) => {
            this.initElement(
              elt,
              elt instanceof HTMLScriptElement &&
                elt.type === "text/hyperscript"
                ? document.body
                : elt,
            );
          },
    ...
    setSymbol(str, context, type, value) {
      if (type === "global") {
        ...
        if (
          this.isHyperscriptContext(context) &&
          !this.isReservedWord(str) &&
          typeof context.locals[str] !== "undefined"
        ) {
          // local scope
          context.locals[str] = value;
        } else {
    ...
    resolveAttribute(root, property) {
      return this.flatGet(
        root,
        property,
        (root, property) => root.getAttribute && root.getAttribute(property),
      );
    }
    ...
    resolveStyle(root, property) {
      return this.flatGet(
        root,
        property,
        (root, property) => root.style && root.style[property],
      );
    }
    ...
    resolveComputedStyle(root, property) {
      return this.flatGet(root, property, (root, property) =>
        getComputedStyle(/** @type {Element} */ (root)).getPropertyValue(
          property,
        ),
      );
    }
        ...
        var traceEntry = {
          ...
          print: function (logger) {
            ...
            for (var i = 0; i < trace.length; i++) {
              maxLen = Math.max(
                maxLen,
                trace[i].meta.feature.displayName.length,
              );
            }
            for (var i = 0; i < trace.length; i++) {
    ...
    beepValueToConsole(element, expression, value) {
      if (
        this.triggerEvent(element, "hyperscript:beep", {
          element,
          expression,
          value,
        })
        ...
        console.log(
          "///_ BEEP! The expression (" +
            Tokens.sourceFor.call(expression).replace("beep! ", "") +
            ") evaluates to:",
          logValue,
          "of type " + typeName,
        );
    ...
    // @ts-ignore
    hyperscriptUrl =
      "document" in globalScope && document.currentScript
  ...
  class Context {
    ...
    constructor(owner, feature, hyperscriptTarget, event, runtime) {
      ...
      ((this.me = hyperscriptTarget), (this.you = undefined));
  ...
  function hyperscriptCoreGrammar(parser) {
    parser.addLeafExpression(
      "parenthesized",
      function (parser, _runtime, tokens) {
        if (tokens.matchOpToken("(")) {
    ...
    parser.addLeafExpression("idRef", function (parser, runtime, tokens) {
      ...
      if (elementId.template) {
        ...
        return {
          ...
          value: value,
          evaluate: function (context) {
            return runtime.getRootNode(context.me).getElementById(value);
          },
        };
      }
    });
    ...
    parser.addLeafExpression("queryRef", function (parser, runtime, tokens) {
      ...
      return {
        ...
        op: function (context, ...args) {
          if (template) {
            return new TemplatedQueryElementCollection(
              queryValue,
              context.me,
              args,
            );
    ...
    parser.addGrammarElement("objectKey", function (parser, runtime, tokens) {
      ...
      if ((token = tokens.matchTokenType("STRING"))) {
        ...
        do {
          token =
            tokens.matchTokenType("IDENTIFIER") || tokens.matchOpToken("-");
    ...
    parser.addIndirectExpression(
      ...
      function (parser, runtime, tokens, root) {
        ...
        if (
          urRoot.type !== "symbol" &&
          urRoot.type !== "attributeRef" &&
          urRoot.type !== "styleRef" &&
          urRoot.type !== "computedStyleRef"
        ) {
          parser.raiseParseError(
            tokens,
            "Cannot take a property of a non-symbol: " + urRoot.type,
          );
        }
        var attribute = urRoot.type === "attributeRef";
        var style =
          urRoot.type === "styleRef" || urRoot.type === "computedStyleRef";
        if (attribute || style) {
        ...
        if (
          ...
          (root.type === "symbol" &&
            ...
            (tokens.currentToken().type === "IDENTIFIER" ||
              tokens.currentToken().type === "ATTRIBUTE_REF" ||
              tokens.currentToken().type === "STYLE_REF"))
        ) {
          if (apostrophe) {
            tokens.requireToken("s");
          }
    ...
    parser.addIndirectExpression(
      ...
      function (parser, runtime, tokens, root) {
        ...
        var conversion = parser
          .requireElement("dotOrColonPath", tokens)
          .evaluate(); // OK No promise
        var propertyAccess = {
          type: "asExpression",
          root: root,
          args: [root],
    ...
    parser.addGrammarElement(
      ...
      function (parser, runtime, tokens) {
        ...
        let stringPosfix =
          tokens.matchAnyToken.apply(tokens, STRING_POSTFIXES) ||
          tokens.matchOpToken("%");
        if (stringPosfix) {
          return {
            type: "stringPostfix",
        ...
        if (tokens.matchToken("s") || tokens.matchToken("seconds")) {
          ...
          tokens.matchToken("ms") ||
          tokens.matchToken("milliseconds")
        ) {
          timeFactor = 1;
        }
        if (timeFactor) {
          ...
          return {
            ...
            op: function (context, val) {
              ...
              if (passed) {
                ...
                throw new Error(
                  "Typecheck failed!  Expected: " + typeName.value,
                );
              }
            },
            evaluate: function (context) {
          ...
          op: function (context, thing, from, inElt, withinElt) {
            ...
            if (css == null) {
              throw (
                ...
                Tokens.sourceFor.apply(thingElt)
              );
            }
    ...
    parser.addGrammarElement(
      ...
      function (parser, runtime, tokens) {
        ...
        var mathOp,
          initialMathOp = null;
        mathOp =
          tokens.matchAnyOpToken("+", "-", "*", "/") ||
          tokens.matchToken("mod");
        while (mathOp) {
      ...
      function (parser, runtime, tokens) {
        ...
        if (operator == null) {
          if (tokens.matchToken("is") || tokens.matchToken("am")) {
            ...
            tokens.matchToken("exist") ||
            tokens.matchToken("exists")
          ) {
            operator = "exist";
            hasRightValue = false;
          } else if (
            tokens.matchToken("matches") ||
            tokens.matchToken("match")
          ) {
            operator = "match";
          } else if (
            tokens.matchToken("contains") ||
            tokens.matchToken("contain")
          ) {
            operator = "contain";
          } else if (
            tokens.matchToken("includes") ||
            tokens.matchToken("include")
          ) {
            operator = "include";
    ...
    parser.addGrammarElement("hyperscript", function (parser, runtime, tokens) {
      ...
      if (tokens.hasMore()) {
        while (
          parser.featureStart(tokens.currentToken()) ||
    ...
    parser.addFeature("on", function (parser, runtime, tokens) {
      ...
      do {
        ...
        if (eventName === "intersection") {
          intersectionSpec = {};
          if (tokens.matchToken("with")) {
            intersectionSpec["with"] = parser
              .requireElement("expression", tokens)
              .evaluate();
          }
          if (tokens.matchToken("having")) {
            do {
              if (tokens.matchToken("margin")) {
                intersectionSpec["rootMargin"] = parser
                  .requireElement("stringLike", tokens)
                  .evaluate();
              } else if (tokens.matchToken("threshold")) {
                intersectionSpec["threshold"] = parser
                  .requireElement("expression", tokens)
                  .evaluate();
              } else {
                parser.raiseParseError(
            ...
            do {
              if (tokens.matchToken("anything")) {
                ...
                if (attribute.value.indexOf("@") == 0) {
                  mutationSpec["attributeFilter"].push(
                    attribute.value.substring(1),
                  );
                } else {
                  parser.raiseParseError(
                    tokens,
          ...
          for (const eventSpec of onFeature.events) {
            ...
            if (eventSpec.elsewhere) {
              ...
              targets = eventSpec.from.evaluate(
                runtime.makeContext(elt, onFeature, elt, null),
              );
            } else {
              targets = [elt];
            }
              ...
              addEventListener.call(target, eventName, function listener(evt) {
                ...
                if (eventSpec.inExpr) {
                  ...
                  while (true) {
                    if (
                      inElement.matches &&
                      inElement.matches(eventSpec.inExpr.css)
                    ) {
      ...
      if (setCmd) {
        ...
        let setFeature = {
          ...
          install: function (target, source) {
            setCmd &&
              setCmd.execute(
                runtime.makeContext(target, setFeature, target, null),
              );
          },
        };
      ...
      var initFeature = {
        ...
        install: function (target, source) {
          let handler = function () {
            start &&
              start.execute(
                runtime.makeContext(target, initFeature, target, null),
              );
          };
          if (immediately) {
            handler();
      ...
      if (tokens.matchToken("for")) {
        ...
        do {
          ...
          if (lookahead.type === "NUMBER" || lookahead.type === "L_PAREN") {
            ...
            events.push({
              name: parser
                .requireElement("dotOrColonPath", tokens, "Expected event name")
                .evaluate(),
          ...
          op: function (context, on) {
            ...
            return new Promise((resolve) => {
              ...
              for (const eventInfo of events) {
                  ...
                  if (eventInfo.args) {
                    for (const arg of eventInfo.args) {
                      context.locals[arg.value] =
                        event[arg.value] ||
                        (event.detail ? event.detail[arg.value] : null);
                    }
                  }
                  if (!resolved) {
                    resolved = true;
                ...
                if (eventInfo.name) {
                  target.addEventListener(eventInfo.name, listener, {
                    once: true,
                  });
    ...
    function parseSendCmd(cmdType, parser, runtime, tokens) {
      ...
      if (
        (cmdType === "send" && tokens.matchToken("to")) ||
    ...
    var makeSetter = function (parser, runtime, tokens, target, value) {
      ...
      if (
        !(attributeWrite || styleWrite || symbolWrite) &&
        target.root == null
      ) {
        parser.raiseParseError(
          tokens,
    ...
    parser.addCommand("if", function (parser, runtime, tokens) {
      ...
      let elseToken =
        tokens.matchToken("else") || tokens.matchToken("otherwise");
      if (elseToken) {
        let elseIfIfToken = tokens.peekToken("if");
        nestedIfStmt =
          elseIfIfToken != null && elseIfIfToken.line === elseToken.line;
        if (nestedIfStmt) {
    ...
    parser.addCommand("append", function (parser, runtime, tokens) {
      ...
      var setter = null;
      if (
        targetExpr.type === "symbol" ||
        targetExpr.type === "attributeRef" ||
        targetExpr.root != null
      ) {
        setter = makeSetter(
          parser,
          runtime,
    ...
    function parsePickRange(parser, runtime, tokens) {
      ...
      rv.from = tokens.matchToken("start")
        ? 0
    ...
    parser.addCommand("pick", (parser, runtime, tokens) => {
      ...
      if (
        tokens.matchToken("item") ||
        tokens.matchToken("items") ||
        tokens.matchToken("character") ||
        tokens.matchToken("characters")
      ) {
      ...
      if (tokens.matchToken("matches")) {
        ...
        if (tokens.matchOpToken("|")) {
          flags =
            "g" + tokens.requireToken("identifier").value.replace("g", "");
        }
    ...
    parser.addCommand("fetch", function (parser, runtime, tokens) {
      ...
      var fetchCmd = {
        ...
        op: function (context, url, args) {
          ...
          let abortListener = context.me.addEventListener(
            "fetch:abort",
            function () {
          ...
          return fetch(url, args)
            .then(function (resp) {
              ...
              if (type === "response") {
                context.result = resp;
                runtime.triggerEvent(context.me, "fetch:afterRequest", {
                  result: resp,
                });
                finished = true;
  ...
  function hyperscriptWebGrammar(parser) {
      ...
      function (parser, runtime, tokens) {
        ...
        while (tokens.hasMore()) {
          if (tokens.matchOpToken("\\")) {
            ...
            stringParts[stringParts.length - 1] += tokens.source.substring(
              tok.start,
              tok.end,
            );
          }
    ...
    var HIDE_SHOW_STRATEGIES = {
      display: function (op, element, arg) {
        if (arg) {
          ...
          const internalData = parser.runtime.getInternalData(element);
          if (
            internalData.originalDisplay &&
            internalData.originalDisplay !== "none"
          ) {
    ...
    var parseShowHideTarget = function (parser, runtime, tokens) {
      ...
      if (
        currentTokenValue.value === "when" ||
        currentTokenValue.value === "with" ||
        parser.commandBoundary(currentTokenValue)
      ) {
    ...
    parser.addLeafExpression("closestExpr", function (parser, runtime, tokens) {
      if (tokens.matchToken("closest")) {
        ...
        if (tokens.currentToken().type === "ATTRIBUTE_REF") {
          var attributeRef = parser.requireElement(
            "attributeRefAccess",
            tokens,
            null,
          );
          ...
          op: function (ctx, to) {
            if (to == null) {
              ...
              runtime.implicitLoop(to, function (to) {
                if (parentSearch) {
                  result.push(
                    to.parentElement ? to.parentElement.closest(css) : null,
                  );
                } else {
      ...
      if (tokens.matchToken("go")) {
        if (tokens.matchToken("back")) {
          ...
          if (tokens.matchToken("url")) {
            ...
            tokens.matchToken("the"); // optional the
            var verticalPosition = tokens.matchAnyToken(
              "top",
              "middle",
              "bottom",
            );
            var horizontalPosition = tokens.matchAnyToken(
              "left",
              "center",
              "right",
            );
            ...
            if (back) {
              ...
              runtime.implicitLoop(to, function (target) {
                ...
                if (plusOrMinus) {
                  ...
                  let actualOffset =
                    plusOrMinus.value === "+" ? offset : offset * -1;
                  ...
                  scrollShim.style.top =
                    boundingRect.top + window.scrollY + offsetY + "px";
                  scrollShim.style.left =
                    boundingRect.left + window.scrollX + offsetX + "px";
  ...
  const runtime_ = new Runtime(),
    lexer_ = runtime_.lexer,
    parser_ = runtime_.parser;
  ...
  function browserInit() {
    ...
    var scripts = Array.from(
      globalScope.document.querySelectorAll(
        "script[type='text/hyperscript'][src]",
      ),
    );
    Promise.all(
   ...
   * @type {Hyperscript}
   */
  const _hyperscript = Object.assign(run, {
    config,
