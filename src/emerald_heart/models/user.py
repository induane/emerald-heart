...
class User(AbstractUser, BaseMixin):
    ...
    #     )
    # )
    # Written out here in a static way so it doens't have to be calculated every time, and so that it is stable
    # doesn't generate too many migrations.
    TIMEZONE_CHOICES = (
    ...
    def server_tzinfo(self) -> zoneinfo.ZoneInfo:
        """Return the local server zoneinfo; useful for converting naive datetime objects."""
        return get_server_tz()

    def datetime_astimezone(self, dt_obj: datetime) -> datetime:
        """Convert a datetime to the users preferred timezone."""
        if is_naive(dt_obj):
            dt_obj = dt_obj.replace(tzinfo=self.server_tzinfo)
        return dt_obj.astimezone(self.tzinfo)
    ...
    def tzinfo(self) -> zoneinfo.ZoneInfo:
        """Return a ZoneInfo instance for the users current timezone."""
        try:
            return zoneinfo.ZoneInfo(self.timezone)
        except Exception:
            LOG.exception("Unable to convert %s to ZoneInfo instance", self.timezone)
            return zoneinfo.ZoneInfo("America/Chicago")

    @property
    def latitude(self) -> float | None:
        if self.current_location:
            return self.current_location.y
        else:
            return None
    ...
    def longitude(self) -> float | None:
        if self.current_location:
            return self.current_location.x
        else:
            return None
    ...
    def coordinates(self) -> Coordinate:
        """
        Return a set of coordinates in longitude, latitude form.
