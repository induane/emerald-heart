...
class ListOutgoingRequests(EmeraldView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        q_obj = Q(source_user=request.user)
        if search:
            search_q_obj = self.get_search_qobj(search, fields=("dest_user__name",))
            q_obj &= search_q_obj
        qs = Request.objects.filter(q_obj)
        return self.render(
            {
                "item_list": paginate(queryset=qs, request=request),
                "column_1_name": "Outgoing Requests",
                "info_url_name": "member-request-info",
                "item_select_url_name": "member-outgoing-request-list-selected",
                "page_number": page,
            }
        )


class ListOutgoingRequestsSelected(EmeraldView):
    """List outgoing requests with a selection."""
    ...
    def get(self, request, id, *args, **kwargs):
        ...
        selected = get_object_or_404(Request, pk=id, source_user=request.user)
        return self.render(
            {
                "item_list": paginate(queryset=qs, request=request),
